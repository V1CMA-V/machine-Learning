...
predictions <- foreach(m = 1:iterations, .combine = cbind) %do% {
  n_samples <- floor(nrow(training) / length_divisor)
  training_positions <- sample(nrow(training), size = n_samples)
  train_pos <- seq_len(nrow(training)) %in% training_positions
...
# El error de la SVM fue superior a la de los modelos lineales,
# probaremos ajustando parametros con la funcion bagging
length_divisor <- 6
iterations <- 5000
predictions <- foreach(m = 1:iterations, .combine = cbind) %do% {
  n_samples <- floor(nrow(training) / length_divisor)
...
# Ajustando los parametros de la SVM
